...
const categories = [
  ...
  { id: 'electrical', icon: Zap, label: 'Electrical', color: 'bg-yellow-100 text-yellow-600', popular: true },
  { id: 'ac', icon: Wind, label: 'AC Service & Repair', color: 'bg-cyan-100 text-cyan-600', popular: true },
  { id: 'cleaning', icon: SprayCan, label: 'Cleaning & Disinfection', color: 'bg-green-100 text-green-600', popular: true },
  { id: 'pest', icon: Bug, label: 'Pest Control', color: 'bg-red-100 text-red-600', popular: false },
  { id: 'painting', icon: PaintBucket, label: 'Painting & Waterproofing', color: 'bg-indigo-100 text-indigo-600', popular: false },
  { id: 'carpentry', icon: Hammer, label: 'Carpentry', color: 'bg-amber-100 text-amber-600', popular: false },
  { id: 'appliance', icon: Package, label: 'Appliance Repair', color: 'bg-purple-100 text-purple-600', popular: false },
  { id: 'renovations', icon: BrickWall, label: 'Renovations & Masonry', color: 'bg-teal-100 text-teal-600', popular: false },
];
          ...
          <div className="flex-1 w-full">
            ...
            <div className="relative mt-3 flex items-center justify-between w-full max-w-sm">
              ...
              {stepOrder.slice(0, 5).map((s, i) => {
                ...
                return (
                  <div key={s} className={`relative z-10 flex items-center justify-center w-6 h-6 rounded-full text-[10px] font-bold transition-all duration-300 shadow-sm ${isActive ? 'bg-[#2563EB] text-white ring-4 ring-blue-100 scale-110' : isPassed ? 'bg-green-500 text-white' : 'bg-white text-gray-400 border border-gray-300'}`}>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
                ...
                <div className="flex-1 pr-4">
                  ...
                  <div className="flex items-center gap-4 mt-2">
                    <div className="flex items-center gap-1 bg-yellow-50 px-2 py-0.5 rounded-md">
                      ...
                      <span className="text-xs font-bold text-yellow-700">{s.rating}</span>
                      <span className="text-[10px] text-yellow-600">({s.reviews})</span>
                    </div>
            ...
            {bookingType === 'scheduled' && (
              <>
                ...
                {selectedDate && selectedTime && (
                  <div className="mt-6 bg-green-50 rounded-xl p-4 flex items-center gap-3">
                    <Calendar className="w-5 h-5 text-green-600" />
                    <div>
                      <p className="text-sm font-medium text-green-800">Slot Selected</p>
                      <p className="text-xs text-green-600">{selectedDate} at {selectedTime}</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setStep('address')}
        ...
        {step === 'address' && (
          <div className="space-y-5">
            ...
            {addressError && (
              <div className="flex items-center gap-2 text-red-600 text-xs mt-1 bg-red-50 p-3 rounded-lg border border-red-200">
          ...
          <div className="space-y-4">
            ...
            {(couponApplied || discount > 0) ? (
              <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
                ...
                <p className="text-sm font-bold text-emerald-700">You are saving ₹{discount} on this order 🎉</p>
              </div>
            ) : (
            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              ...
              <div className="px-5 py-4 space-y-3">
                ...
                {couponApplied && (
                  <p className="text-green-600 text-xs flex items-center gap-1.5 font-semibold">
            ...
            <div className="flex flex-wrap gap-3 text-xs text-gray-500">
              ...
              <div className="flex items-center gap-1.5"><CheckCircle className="w-3.5 h-3.5 text-green-500" /> 90-day warranty</div>
