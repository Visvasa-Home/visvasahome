...
const TOUR_KEY = 'visvasahome_tour_completed';
...
const tourSteps: TourStep[] = [
  ...
  },
  {
    ...
    description: 'Track your assigned professional live on the map, chat with them, rate services, and manage your booked AMC schedules from your profile.',
    position: 'bottom'
  }
];
...
export function InteractiveTour({ isAuthenticated }: InteractiveTourProps) {
  const [showWelcome, setShowWelcome] = useState(false);
  const [isOpen, setIsOpen] = useState(false);
  const [activeStep, setActiveStep] = useState(0);
  const [highlightRect, setHighlightRect] = useState<DOMRect | null>(null);

  // Only trigger tour once — when user logs in for the first time ever
  useEffect(() => {
    if (isAuthenticated && !localStorage.getItem(TOUR_KEY)) {
      // Small delay so page settles after login redirect
      const t = setTimeout(() => setShowWelcome(true), 800);
      return () => clearTimeout(t);
    }
  }, [isAuthenticated]);

  // Recalculate highlight rect on scroll/resize
  useEffect(() => {
    if (!isOpen) return;
  ...
  // Nothing to render if user hasn't just logged in or tour is done
  if (!showWelcome && !isOpen) return null;
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full border border-gray-100 shadow-2xl text-center relative overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="space-y-3">
              <button
                ...
                className="w-full py-3.5 bg-gradient-to-r from-blue-600 to-blue-600 text-white rounded-2xl font-bold hover:opacity-95 shadow-md shadow-blue-500/20 active:scale-98 transition-all flex items-center justify-center gap-2 text-sm"
              >
          ...
          <div className="fixed bottom-6 left-1/2 -translate-x-1/2 max-w-sm w-full mx-4 bg-white rounded-2xl shadow-2xl border border-gray-100 p-5 z-[99992] flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-6 duration-300">
            ...
            <div className="flex items-center justify-between pt-1 mt-1 border-t border-gray-50">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={handleNext}
                  className="flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-600 text-white hover:opacity-95 rounded-xl text-xs font-black transition-all shadow-xs"
                >
