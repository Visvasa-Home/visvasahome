...
const homeServices = [
  ...
  { icon: Refrigerator, label: 'Appliances', color: 'bg-blue-100 text-blue-600', page: 'appliance-repair' },
  { icon: Bug, label: 'Pest Control', color: 'bg-blue-100 text-blue-650', page: 'pest-control' },
  { icon: Sparkles, label: 'Cleaning', color: 'bg-blue-100 text-blue-600', page: 'cleaning-services' },
];

// ── 2. Construction & Trades ──
const constructionServices = [
  { icon: Droplets, label: 'Plumbing', color: 'bg-blue-100 text-blue-600', page: 'plumbing-services' },
  { icon: Zap, label: 'Electrical', color: 'bg-blue-100 text-blue-600', page: 'electrical-services' },
  { icon: Wind, label: 'AC & HVAC', color: 'bg-blue-100 text-blue-600', page: 'ac-services' },
  { icon: PaintBucket, label: 'Painting', color: 'bg-blue-100 text-blue-600', page: 'painting-services' },
  { icon: Hammer, label: 'Carpentry', color: 'bg-blue-100 text-blue-650', page: 'carpentry-services' },
  ...
  { icon: Home, label: 'Roofing', color: 'bg-blue-100 text-blue-700', page: 'roofing-services' },
];

// ── 3. Design & Finishing ──
const designServices = [
  { icon: Sofa, label: 'Interior Design', color: 'bg-blue-100 text-blue-600', page: 'interior-design' },
  { icon: Trees, label: 'Landscaping', color: 'bg-blue-100 text-blue-700', page: 'landscaping-services' },
  { icon: Frame, label: 'Flooring', color: 'bg-blue-100 text-blue-600', page: 'flooring-services' },
];

// ── 4. Specialty & Personal Care ──
const specialtyServices = [
  { icon: Scissors, label: 'At-Home Salon', color: 'bg-blue-100 text-blue-650', page: 'beauty-services' },
  { icon: Heart, label: 'Wellness', color: 'bg-blue-100 text-blue-500', page: 'wellness-services' },
  { icon: Baby, label: 'Care & Support', color: 'bg-blue-100 text-blue-600', page: 'care-services' },
  { icon: PartyPopper, label: 'Event Setup', color: 'bg-blue-100 text-blue-700', page: 'event-services' },
];
...
export function MenuPage({ onBack, onNavigate, isAuthenticated, onLogin }: MenuPageProps) {
  return (
    <div className="min-h-screen bg-gray-50 pb-20 lg:pb-0">
      ...
      <div className="w-full md:max-w-3xl lg:max-w-5xl xl:max-w-7xl mx-auto px-4 py-5 space-y-6">
        ...
        <section>
          ...
          <button
            ...
            className="w-full mt-3 py-3 bg-blue-700 hover:bg-blue-800 text-white rounded-xl font-semibold text-sm transition-colors"
          >
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-blue-700 bg-blue-50 border border-blue-100 px-2 py-0.5 rounded-full font-bold">Aesthetics & Finish</span>
          </div>
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            {designServices.map((svc, idx) => (
              <button
                key={svc.page}
                onClick={() => onNavigate(svc.page)}
                className={`w-full flex items-center justify-between px-4 py-3.5 hover:bg-blue-50 transition-colors ${idx < designServices.length - 1 ? 'border-b border-gray-100' : ''
                  }`}
              >
                <div className="flex items-center gap-3">
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-blue-700 bg-blue-50 border border-blue-100 px-2 py-0.5 rounded-full font-bold">At-Home Services</span>
          </div>
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            {specialtyServices.map((svc, idx) => (
              <button
                key={svc.page}
                onClick={() => onNavigate(svc.page)}
                className={`w-full flex items-center justify-between px-4 py-3.5 hover:bg-blue-50 transition-colors ${idx < specialtyServices.length - 1 ? 'border-b border-gray-100' : ''
                  }`}
              >
                <div className="flex items-center gap-3">
                  <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${svc.color}`}>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-blue-600 bg-blue-50 border border-blue-100 px-2 py-0.5 rounded-full font-bold">Annual Contracts</span>
          </div>
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            {amcPlans.map((plan, idx) => (
              <button
                ...
                className={`w-full flex items-center justify-between px-4 py-3.5 hover:bg-blue-50 transition-colors ${idx < amcPlans.length - 1 ? 'border-b border-gray-100' : ''
                  }`}
              >
                <div>
