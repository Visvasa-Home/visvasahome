        ...
        {success && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6 animate-slide-up">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-blue-900 mb-1">Verification Email Sent!</p>
                ...
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span
                    className={`text-xs font-semibold ${
                      passwordStrength.score >= 3 ? 'text-blue-600' : 'text-blue-600'
                    }`}
                  >
              ...
              <div className="mt-2 flex items-center gap-2">
                {password === confirmPassword ? (
                  <>
                    ...
                    <span className="text-xs text-blue-600 font-medium">Passwords match</span>
                  </>
                ) : (
                  <>
                    <AlertCircle className="w-4 h-4 text-blue-600" />
                    <span className="text-xs text-blue-600 font-medium">Passwords do not match</span>
                  </>
                )}
              </div>
            )}
        ...
        <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-xl">
          ...
          <ul className="space-y-1 text-xs text-blue-800">
            <li className="flex items-center gap-2">
              <div
                className={`w-1.5 h-1.5 rounded-full ${password.length >= 12 ? 'bg-blue-500' : 'bg-gray-400'}`}
              />
              At least 12 characters long
            </li>
            <li className="flex items-center gap-2">
              <div
                className={`w-1.5 h-1.5 rounded-full ${/[a-z]/.test(password) && /[A-Z]/.test(password) ? 'bg-blue-500' : 'bg-gray-400'}`}
              />
              Both uppercase and lowercase letters
            </li>
            <li className="flex items-center gap-2">
              <div
                className={`w-1.5 h-1.5 rounded-full ${/\d/.test(password) ? 'bg-blue-500' : 'bg-gray-400'}`}
              />
              At least one number
            </li>
            <li className="flex items-center gap-2">
              <div
                className={`w-1.5 h-1.5 rounded-full ${/[^a-zA-Z0-9]/.test(password) ? 'bg-blue-500' : 'bg-gray-400'}`}
              />
