...
export const AMCPackagesPage = ({ onBack, onNavigate }: AMCPackagesPageProps) => {
  ...
  const getTierColor = (tier: string): string => {
    switch (tier) {
      case 'platinum': return 'bg-blue-100 text-blue-900 border-blue-300';
      case 'gold': return 'bg-blue-100 text-blue-900 border-blue-300';
  ...
  const PackageCard = ({ pkg }: { pkg: AMCPackage }) => (
    <Card className="relative overflow-hidden hover:shadow-lg transition-shadow">
      {pkg.tier === 'platinum' && (
        <div className="absolute top-0 right-0 bg-gradient-to-l from-blue-600 to-blue-400 text-white text-xs px-3 py-1 rounded-bl-lg">
          POPULAR
        </div>
      )}
