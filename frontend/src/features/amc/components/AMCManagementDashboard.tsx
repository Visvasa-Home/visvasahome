...
const statusConfig: Record<string, { label: string; color: string; bg: string; dot: string }> = {
  completed: { label: 'Completed', color: 'text-blue-700', bg: 'bg-blue-50 border-blue-100', dot: 'bg-blue-500' },
  upcoming: { label: 'Upcoming', color: 'text-blue-700', bg: 'bg-blue-50 border-blue-100', dot: 'bg-blue-500 animate-pulse' },
  scheduled: { label: 'Scheduled', color: 'text-blue-700', bg: 'bg-blue-50 border-blue-100', dot: 'bg-blue-500' },
      ...
      <div className="bg-gradient-to-br from-slate-900 to-blue-950 text-white">
        <div className="max-w-3xl mx-auto px-4 pt-4 pb-6">
          ...
          {isNearExpiry && !isExpired && (
            <div className="bg-blue-500/20 border border-blue-400/40 rounded-2xl p-3 mb-4 flex items-start gap-3">
              <AlertTriangle className="w-5 h-5 text-blue-300 flex-shrink-0 mt-0.5" />
              <div className="flex-1">
                <p className="text-blue-100 font-black text-sm">Plan Nearing Expiry!</p>
              ...
              <button
                ...
                className="bg-blue-400 hover:bg-blue-300 text-blue-900 font-black text-xs px-3 py-1.5 rounded-xl flex-shrink-0 transition-all"
              >
                Renew →
              </button>
            </div>
          ...
          {isExpired && (
            <div className="bg-blue-500/20 border border-blue-400/40 rounded-2xl p-3 mb-4 flex items-start gap-3">
              <XCircle className="w-5 h-5 text-blue-300 flex-shrink-0 mt-0.5" />
              <div className="flex-1">
                <p className="text-blue-100 font-black text-sm">Plan Lapsed</p>
                <p className="text-blue-200 text-xs font-semibold mt-0.5">
                  Your AMC has expired. Future visits are charged at pay-per-visit rates.
                </p>
              </div>
              <button onClick={() => setActiveTab('renewal')} className="bg-blue-400 text-white font-black text-xs px-3 py-1.5 rounded-xl flex-shrink-0">
                Renew
              </button>
            </div>
          )}
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className="text-[10px] text-blue-300 font-extrabold uppercase tracking-widest">
            ...
            <span className={`text-[10px] font-extrabold uppercase px-3 py-1.5 rounded-xl flex items-center gap-1 flex-shrink-0 ${
              isExpired ? 'bg-blue-500/30 text-blue-200' : 'bg-blue-500/20 text-blue-300 border border-blue-500/30'
            }`}>
          ...
          <div className="grid grid-cols-4 gap-2 mt-5">
            {[
              ...
              { label: 'Remaining', value: visitsRemaining, color: 'text-blue-300' },
              { label: 'Days Left', value: isExpired ? '0' : daysLeft, color: isNearExpiry ? 'text-blue-300' : 'text-white' },
            ].map(s => (
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
              ...
              <a
                ...
                className="bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs px-4 py-2.5 rounded-xl flex items-center gap-1.5 transition-colors"
              >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className={`text-[10px] font-black px-2 py-0.5 rounded-full flex-shrink-0 ml-2 ${
                          appliance.status === 'good' ? 'text-blue-700 bg-blue-50' :
                          appliance.status === 'warning' ? 'text-blue-700 bg-blue-50' :
                      ...
                      <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-500 ${
                            appliance.health >= 70 ? 'bg-blue-500' :
                            appliance.health >= 40 ? 'bg-blue-400' : 'bg-blue-500'
                          }`}
                    ...
                    {visit.rating !== null && (
                      <div className="flex items-center gap-1">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <Star key={i} className={`w-3.5 h-3.5 ${i < (visit.rating || 0) ? 'text-blue-400 fill-amber-400' : 'text-gray-200 fill-gray-200'}`} />
                        ))}
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      >
                        <AlertTriangle className={`w-5 h-5 mb-2 ${requestType === 'breakdown' ? 'text-blue-500' : 'text-gray-400'}`} />
                        <p className={`text-sm font-extrabold ${requestType === 'breakdown' ? 'text-blue-600' : 'text-gray-700'}`}>
                          Breakdown Visit
                        </p>
                  ...
                  {serviceRequests.map(req => (
                    <div key={req.id} className="flex items-start gap-3 p-3 bg-gray-50 rounded-2xl">
                      <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 ${
                        req.type === 'breakdown' ? 'bg-blue-100' : 'bg-blue-100'
                      }`}>
                        {req.type === 'breakdown' ? <AlertTriangle className="w-4 h-4 text-blue-500" /> : <Calendar className="w-4 h-4 text-blue-500" />}
                      </div>
                      <div className="flex-1 min-w-0">
                      ...
                      <span className="text-[10px] font-bold text-blue-700 bg-blue-50 px-2 py-0.5 rounded-full flex-shrink-0">
                        {req.status}
                      </span>
                    </div>
                  ))}
        ...
        {activeTab === 'renewal' && (
          <div className="space-y-4">
            ...
            <div className={`rounded-3xl p-5 border ${
              isExpired ? 'bg-blue-900 border-blue-800 text-white' :
              isNearExpiry ? 'bg-blue-50 border-blue-200' :
              'bg-blue-50 border-blue-200'
            }`}>
              <div className="flex items-center gap-3 mb-3">
                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0 ${
                  isExpired ? 'bg-blue-700' : isNearExpiry ? 'bg-blue-100' : 'bg-blue-100'
                }`}>
                  {isExpired ? <XCircle className="w-5 h-5 text-blue-200" /> :
                   isNearExpiry ? <AlertTriangle className="w-5 h-5 text-blue-600" /> :
                   <Shield className="w-5 h-5 text-blue-600" />}
                </div>
                <div>
                  <p className={`text-[10px] font-extrabold uppercase tracking-widest ${
                    isExpired ? 'text-blue-300' : isNearExpiry ? 'text-blue-500' : 'text-blue-600'
                  }`}>
              ...
              <p className={`text-xs font-semibold ${isExpired ? 'text-blue-200' : isNearExpiry ? 'text-blue-700' : 'text-blue-700'}`}>
                {isExpired
            ...
            {!renewalConfirmed ? (
              <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-5">
                ...
                <div className="grid grid-cols-1 gap-3">
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-3">
                        ...
                        <span className="text-xs font-bold text-blue-600 bg-blue-50 border border-blue-100 px-2 py-0.5 rounded-full">No gap in coverage</span>
                      </div>
                    </div>
                  ...
                  <button
                    ...
                    className="w-full p-5 rounded-2xl border-2 border-gray-200 hover:border-blue-300 text-left flex items-start gap-4 transition-all hover:shadow-md"
                  >
                    <div className="w-11 h-11 bg-blue-50 rounded-2xl flex items-center justify-center flex-shrink-0">
              ...
              <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-8 text-center">
                <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-5 ${
                  renewalChoice === 'renew' ? 'bg-blue-50' : 'bg-gray-100'
                }`}>
                  {renewalChoice === 'renew'
            ...
            {!renewalConfirmed && (
              <div className="bg-slate-900 rounded-3xl p-5 text-white">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <p className="text-sm font-black text-blue-400 mb-2">✅ With Renewal</p>
                    {[
                  ...
                  <div>
                    <p className="text-sm font-black text-blue-400 mb-2">❌ If Lapsed</p>
                    {[
                      'Pay-per-visit rates',
