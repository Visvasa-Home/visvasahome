      ...
      <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden animate-slide-up">
        ...
        <div className="p-5">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-gray-800 flex items-center gap-2">
                        {m.label}
                        {m.recommended && (
                          <span className="bg-blue-100 text-blue-700 text-xs px-1.5 py-0.5 rounded-full">Recommended</span>
                        )}
                      </div>
          ...
          {step === 'failure' && (
            <div className="py-8 text-center">
              <div className="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <X className="w-9 h-9 text-blue-500" />
              </div>
...
function TierBadge({ tier }: { tier: string }) {
  const config: Record<string, { label: string; cls: string }> = {
    bronze:   { label: '🥉 Bronze',   cls: 'bg-blue-100 text-blue-800' },
    silver:   { label: '🥈 Silver',   cls: 'bg-gray-100 text-gray-700' },
    gold:     { label: '🥇 Gold',     cls: 'bg-blue-100 text-blue-800' },
    platinum: { label: '💎 Platinum', cls: 'bg-blue-100 text-blue-800' },
  };
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        {tab === 'wallet' && (
          <div>
            ...
            {onNavigate && (
              <div className="bg-gradient-to-br from-blue-50 to-blue-50 border border-blue-200 rounded-xl p-4 mb-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-blue-900 mb-1">Loyalty Rewards</p>
                    {loadingLoyalty ? (
                      <p className="text-xs text-blue-600">Loading points…</p>
                    ) : (
                  ...
                  {walletTxns.slice(0, 8).map(t => (
                    <div key={t.id} className="flex items-center gap-3 px-4 py-3.5">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0
                        ${t.type === 'credit' ? 'bg-blue-100' : 'bg-blue-100'}`}>
                        {t.type === 'credit'
                      ...
                      <div className={`font-semibold text-sm ${t.type === 'credit' ? 'text-blue-600' : 'text-gray-700'}`}>
        ...
        {tab === 'referral' && (
          <div className="space-y-5">
            {/* Hero */}
            <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl p-6 text-white text-center relative overflow-hidden">
              <div className="absolute inset-0 opacity-10">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              ...
              <div className="space-y-4">
                {[
                  ...
                  { icon: CheckCircle, label: 'Friend signs up', desc: 'Your friend creates an account using your code', color: 'bg-blue-100 text-blue-600' },
                  ...
                  <div key={i} className="flex items-center gap-3 px-4 py-3.5 border-b border-gray-100 last:border-0">
                    ...
                    <div className="text-right">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full
                        ${r.status === 'rewarded' ? 'bg-blue-100 text-blue-700'
                        : r.status === 'completed' ? 'bg-blue-100 text-blue-700'
                        : 'bg-blue-100 text-blue-700'}`}>
                        {r.status === 'rewarded' ? 'Rewarded' : r.status === 'completed' ? 'Completed' : 'Pending'}
                      </span>
                      {r.status === 'rewarded' && (
                        <div className="text-xs text-blue-600 font-bold mt-0.5">+₹{r.creditAmount}</div>
                      )}
                    </div>
                  </div>
                ))
