...
const TIERS = [
  ...
  { name: 'Gold', min: 1000, max: 4999, icon: Star, color: '#d97706', bg: 'from-blue-400 to-blue-600', light: '#fffbeb' },
  { name: 'Platinum', min: 5000, max: 9999, icon: Crown, color: '#7c3aed', bg: 'from-blue-500 to-blue-700', light: '#f5f3ff' },
  { name: 'Diamond', min: 10000, max: Infinity, icon: Trophy, color: '#0ea5e9', bg: 'from-blue-400 to-blue-600', light: '#f0f9ff' },
];
      ...
      <div className="max-w-2xl mx-auto px-4 pb-16">
        ...
        >
          ...
          <div className="relative p-6">
            ...
            {balance.expiringPoints > 0 && (
              <div className="bg-blue-400/20 border border-blue-300/30 rounded-xl p-3 mb-4">
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-5 mb-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="ml-auto text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">Active</span>
          </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
                  copied ? 'bg-blue-500 text-white' : 'bg-blue-600 text-white hover:bg-blue-700'
                }`}
              >
          ...
          <button
            onClick={handleShare}
            className="w-full flex items-center justify-center gap-2 py-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-2xl text-sm font-bold transition-all"
          >
          ...
          {referralStats && (
            <div className="grid grid-cols-3 gap-3 mt-4 pt-4 border-t border-gray-100">
              {[
                ...
                { label: 'Converted', value: referralStats.completedReferrals, color: 'text-blue-600' },
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="bg-white rounded-2xl border border-gray-100 p-4 flex items-center gap-4 shadow-sm hover:shadow-md transition-shadow">
                ...
                <span className="text-blue-600 font-black text-sm flex-shrink-0">{item.points}</span>
              </div>
            ))}
          </div>
        )}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        {reward.popular && (
                          <span className="text-[9px] font-black text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded-full flex-shrink-0">HOT</span>
                        )}
                      </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            {referralHistory.length > 0 ? (
                  ...
                  <div key={ref.id} className="flex items-center gap-3 p-4">
                    ...
                    <div className="text-right flex-shrink-0">
                      <span className={`inline-block text-[10px] font-bold px-2 py-1 rounded-full ${
                        ref.status === 'rewarded' ? 'bg-blue-50 text-blue-700' :
                      ...
                      {ref.status === 'rewarded' && (
                        <p className="text-blue-600 text-xs font-bold mt-1">+₹{ref.creditAmount}</p>
                      )}
                    </div>
                  </div>
                ))}
              ...
              <div className="text-center py-12 px-4">
                ...
                <button
                  onClick={handleShare}
                  className="mt-4 px-5 py-2 bg-blue-500 text-white rounded-xl text-xs font-bold hover:bg-blue-600 transition-colors"
                >
                  Share on WhatsApp
                </button>
              </div>
                ...
                {transactions.map((txn) => (
                  <div key={txn.id} className="flex items-start gap-3 p-4">
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${
                      txn.points > 0 ? 'bg-blue-50' : 'bg-blue-50'
                    }`}>
                      {txn.points > 0
                    ...
                    <p className={`font-black text-sm flex-shrink-0 ${txn.points > 0 ? 'text-blue-600' : 'text-blue-500'}`}>
